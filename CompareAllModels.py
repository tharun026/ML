...
from ms_validation import learning_curve
...
#Load The Dataset
url = "input/data.csv"
features = ['transactions', 'frequency', 'duration', 'diversity','class']
#features = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','class']
...
def predict(X, vectorize, model_name):
    if vectorize==1:  
        X, Y = get_XY("input/test.csv", vectorize, ['key','temp']); #print(X)
        X = tex2vec(X); X = X.toarray(); #print(X)
    model = pickle.load(open('models/'+model_name+'.pickle','rb'))
